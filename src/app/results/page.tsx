...
import { getSessionAnswers } from '@/lib/mock-data';
  ...
  const getPersonalityInsight = () => {
    if (yesPercentage >= 80) {
      return {
        ...
        description: 'You have an extraordinary capacity for empathy and service. Your heart naturally gravitates toward helping others.',
        emoji: '💖',
      };
    } else if (yesPercentage >= 60) {
      return {
        title: 'The Mindful Helper',
        description: 'You balance self-care with care for others beautifully. You know when to give and when to preserve your energy.',
        emoji: '🌟',
      };
    } else if (yesPercentage >= 40) {
      return {
        title: 'The Thoughtful Observer',
        description: 'You approach helping with discernment. You think carefully before committing, which shows wisdom.',
        emoji: '🦉',
      };
    } else {
      return {
        title: 'The Self-Aware Individual',
        description: 'You value honesty about your boundaries. Knowing your limits is the first step to genuine giving.',
        emoji: '🔮',
      };
    }
  };
